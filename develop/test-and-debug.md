...
description: Leer hoe u uw Partner Center-integratie-sandboxaccount (en gerelateerde tokens) kunt gebruiken om uw code te testen en fouten op te sporen, zodat er niet per ongeluk nieuwe kosten in rekening worden gebracht.
ms.date: 09/11/2018
...
Als u uw code wilt testen, moet u uw integratie-sandboxaccount gebruiken in Partner Center (en de bijbehorende tokens), zodat u niet per ongeluk nieuwe kosten in rekening brengt die uw bedrijf moet betalen. Zie API-toegang instellen in Partner Center voor meer informatie over [deze Test-In-Production-omgeving (TiP).](set-up-api-access-in-partner-center.md)

## <a name="integration-sandbox-constraints"></a>Beperkingen voor de integratie-sandbox

Als u geautomatiseerde buildverificatietests, tests in productie of handmatige tests in de integratie-sandbox voert, kunt u de maximumlimieten voor de integratie-sandbox bereiken. Deze limieten zijn 75 klanten, 5 abonnementen per klant en 25 licenties per abonnement.
...
| Dagelijks niet-gebilled gebruiksbestand   | Nee                       | Ja                      |


### <a name="azure-plan"></a>Azure-abonnement

Standaard kunnen partners geen Azure-abonnementen inrichten met behulp van hun sandbox-accounts. Partners die dit willen doen met hun sandbox-account, moeten toegang aanvragen. Als u toegang wilt aanvragen, neem dan contact op met Microsoft-account manager of zakelijke contactpersoon. Partners die eerder toegang hebben aangevraagd voor het inrichten van Microsoft Azure-abonnementen (MS-AZR-0145P) in hun sandbox-accounts, hoeven geen toegang opnieuw aan te melden. Ze krijgen automatisch toegang tot het inrichten van Azure-abonnementen.
...
- Elk sandbox-partneraccount kan maximaal 10 Azure-plannen hebben voor alle tenants van klanten (ongeacht hoe de plannen worden verdeeld over de klanten).

- Een directe factuurpartner kan maximaal één Azure-plan per klant-tenant maken.

- Een indirecte provider kan maximaal drie Azure-abonnementen per klantten tenant maken (voor verschillende indirecte resellers die zijn opgegeven als Partner-of-Record).
...
### <a name="cloud-solution-provider-csp-azure-subscription-offers"></a>aanbiedingen Cloud Solution Provider Azure-abonnement (CSP)

Aanbiedingen voor CSP Azure-abonnementen zijn niet meer standaard beschikbaar voor sandbox-accounts. Dit zijn onder andere MS-AZR-0146P, MS-AZR-DE-0146P en MS-AZR-USGOV-0146P voor CSP Azure-abonnementen in respectievelijk Microsoft Public Cloud, German Cloud en Government Cloud. Partners die toegang nodig hebben tot deze aanbiedingen met hun sandbox-account, moeten toegang aanvragen. Als u toegang wilt aanvragen, kunt u dit bespreken Microsoft-account manager of zakelijke contactpersoon.
...
> [!Important]
> Alle bestaande CSP Azure-abonnementen die zijn ingericht met sandbox-accounts vóór 1 augustus 2018, worden niet meer ondersteund en worden van 16 oktober tot 31 oktober 2018 door Microsoft in de inrichting van de inrichting verwijderd. Nadat de abonnementen zijn verwijderd, kunnen ze niet opnieuw worden ingeschakeld en zijn gekoppelde gegevens niet meer toegankelijk. Partners die waardevolle gegevens hebben opgeslagen onder deze abonnementen, moeten vóór 16 oktober 2018 een back-up van de gegevens maken.

### <a name="azure-reserved-vm-instance"></a>Gereserveerde VM-instantie van Azure

Als u een [gereserveerde VM-instantie](purchase-azure-reservations.md) van Azure koopt met uw sandbox-account, bent u beperkt tot twee VM-exemplaren per klant. U kunt ook alleen selecteren in de volgende product-SKU's voor gereserveerde VM-instanties van Azure:
...
Nadat u in productie een abonnement hebt gemaakt op [SaaS-producten](create-subscription-azure-marketplace-products.md)op de commerciële marketplace, moet u een gepersonaliseerde activeringskoppeling ophalen van Partner Center en naar de site van de uitgever gaan om het installatieproces te voltooien. Abonnementsfacturering begint pas nadat de installatie is voltooid.

In de CSP-sandboxomgeving is er geen integratie met ISV's. Als u een activeringskoppeling probeert op te halen Partner Center, wordt er een dummykoppeling geretourneerd. U kunt deze dummykoppeling niet gebruiken om het installatieproces op de site van de uitgever te voltooien. Zie Activate [a sandbox subscription for commercial marketplace products](activate-sandbox-subscription-azure-marketplace-products.md) (Een sandbox-abonnement activeren voor commerciële marketplace-producten) als u het sandbox-account voor integratie wilt gebruiken om de facturering voor abonnementen op SaaS-producten op de commerciële marketplace te testen. Abonnementsfacturering begint na een geslaagde activering.
...
- Gebruik een hulpprogramma voor netwerk traceer, zodat u uw aanvraag, het antwoord en kunt zien of er fouten zijn in de HTTP-statuscode in het antwoord. Zie REST-foutcodes voor Partner Center informatie over [foutafhandeling.](error-codes.md)

- Gebruik een nieuwe correlatie-id voor elke aanroep naar de Partner Center REST API. Deze praktijk zorgt voor betere logboekregistratie en helpt tijdens het debuggen. Zie REST-headers [Partner Center meer informatie.](headers.md)
